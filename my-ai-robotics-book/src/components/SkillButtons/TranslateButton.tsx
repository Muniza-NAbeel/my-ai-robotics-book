...
import { TranslateButtonProps, SkillResponse, API_BASE_URL } from './types';
...
export function TranslateButton({ text }: TranslateButtonProps) {
  ...
  const handleClick = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/skills/translate`, {
        method: 'POST',
        headers: {
    ...
    <div className={styles.skillButtonContainer}>
      ...
      {result && (
        <div className={styles.resultContainer}>
          <pre className={`${styles.resultText} ${styles.rtlText}`}>{result}</pre>
        </div>
      )}
